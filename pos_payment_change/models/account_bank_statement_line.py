...
class AccountBankStatementLine(models.Model):
    ...
    def cash_draw_statement_line(self):
        ...
        for st_line in self:
            ...
            if st_line.account_id and st_line.id not in already_done_stmt_line_ids:
                ...
                balance_line = st_line._prepare_reconciliation_move_line(
                    move_vals,
                    -aml_dict["debit"] if st_line.amount < 0 else aml_dict["credit"],
                )
                # Replace the account
                if (
                    st_line.journal_id.type == "cash"
                    and st_line.journal_id.change_account_id
                ):
                    balance_line.update(
    ...
    def cash_change_counterpart_creation(self):
        ...
        payment_methods = (
            (total > 0)
            and st_line.journal_id.inbound_payment_method_ids
            or st_line.journal_id.outbound_payment_method_ids
        )
