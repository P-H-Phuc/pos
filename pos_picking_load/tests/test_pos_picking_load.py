...
class TestPosPickingLoad(TransactionCase):
    def test_pos_picking_load(self):
        ...
        sale_order.picking_ids.picking_type_id.write({"available_in_pos": True})

        # Configure Point of sale
        config = self.env.ref("point_of_sale.pos_config_main").copy()
        config.write({"picking_type_id": sale_order.picking_ids.picking_type_id.id})

        # Open a new session
        config.open_session_cb()
        session = self.env["pos.session"].search([("config_id", "=", config.id)])[0]
        ...
        self.assertEqual(
            pos_order.picking_id.group_id, pos_order.origin_picking_id.group_id
        )
